...
const TwitterTestimonials = () => {
  ...
  const rows = [tweets.slice(0, 4), tweets.slice(4, 8), tweets.slice(8, 12)];
    ...
    <div className="relative mt-20 mb-20">
      <div className="relative mt-2 mb-8 z-10 max-w-5xl mx-auto flex flex-col justify-center">
        ...
        {rows.map((row, rowIndex) => (
          <div key={rowIndex} className="mt-1 relative ">
            {/* Left Gradient */}
            <div
              className="absolute inset-y-0 left-0 bg-gradient-to-r from-slate-100 to-transparent z-20"
              style={{ width: "20%" }}
            />
            {/* Right Gradient */}
            <div
              className="absolute inset-y-0 right-0 bg-gradient-to-l from-slate-100 to-transparent z-20"
              style={{ width: "20%" }}
            />
            <Marquee
              velocity={isMobile ? 10 : 20 + rowIndex * 5} 
            ...
            >
              {row.map((tweet, index) => (
                <div
                  className="min-w-[300px] md:min-w-[400px] py-4"
                  key={index}
                >
                  <Tweet
                    avatar={tweet.avatar}
